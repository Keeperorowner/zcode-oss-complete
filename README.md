...
> 社区完整化构建分支，基底为 [`zai-org/ZCode`](https://github.com/zai-org/ZCode)（Apache-2.0）。
> 上游把第一方源码里的 `@zcode/zcode-cua` Computer Use 运行时裁成了桩，本分支按 `broker-server.d.ts` 契约补齐；
> 并补回被裁的包构建脚本与 `swift-bridge` 探测。**客户端 UI 与上游逐字节一致**，活动权益由官网 webview 承载。
> 版权归原作者所有；上游 `LICENSE` / `NOTICE.md` / `THIRD-PARTY-NOTICES.md` 全部保留。
> 内置插件层不随本仓库分发，本机自用走 `scripts/apply-local-plugins.ps1`。
